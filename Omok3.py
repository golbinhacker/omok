...
class Omok(object):
    ...
    def draw_stone(self, x, y, coord):
        ...
        if self.coords:
            ...
            self.draw_image(self.id % 2, x, y)
            if self.is_show:
                self.show_number(x, y, self.id % 2 + 1, self.id - 1)
        self.coords.append(coord)

        x, y = coord
        if self.is_show:
            self.draw_image(self.turn - 1, x, y)
            self.show_number(x, y, self.turn + 2, self.id)
        else:
            self.draw_image(self.turn + 1, x, y)
            
        if self.check_gameover(coord):
...
class Menu(object):
    ...
    def draw_menu(self):
        ...
        self.new_rect = self.make_text(self.font, '판 업ㅍ', blue, None, top - 30, left)
        self.quit_rect = self.make_text(self.font, '안해 안해 ', blue, None, top, left)
        self.show_rect = self.make_text(self.font, '번호 숨기기 ', blue, None, top - 60, left)
        self.undo_rect = self.make_text(self.font, '한 수만 무르자', blue, None, top - 150, left)
        self.uall_rect = self.make_text(self.font, '다 무르기', blue, None, top - 120, left)
        self.redo_rect = self.make_text(self.font, '다시놓기', blue, None, top - 90, left)
